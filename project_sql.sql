...
SELECT country, `year` AS GDP_year, GDP, population, ROUND(GDP / population, 2) AS GDP_per_capita, gini
FROM economies e
WHERE country IN (
	SELECT country
	FROM countries c
...
SELECT tkg.payroll_year, tkg.industry_branch_code, tkg.branch, tkg.average_salary_czk, tkg2.average_salary_czk AS prev_year_average_salary_czk,
...
WITH table_growth_rate AS (SELECT tkg.price_year, tkg.category_code, tkg.name, tkg.average_price_czk, tkg2.average_price_czk AS prev_year_average_price_czk,
...
SELECT category_code, name, ROUND(avg(price_growth_percent), 2) AS average_growth_rate
...
WITH table_comparison AS (SELECT tkg.price_year, tkg.category_code, tkg.name, tkg.average_price_czk, tkg2.average_price_czk AS prev_year_average_price_czk,
	ROUND((tkg.average_price_czk - tkg2.average_price_czk) / tkg2.average_price_czk * 100, 2) AS price_growth_percent,
	tkg.industry_branch_code, tkg.branch, tkg.average_salary_czk, tkg2.average_salary_czk AS prev_year_average_salary_czk,
	ROUND((tkg.average_salary_czk - tkg2.average_salary_czk) / tkg2.average_salary_czk * 100, 2) AS salary_growth_percent
FROM t_kristyna_grouslova_project_sql_primary_final tkg
JOIN t_kristyna_grouslova_project_sql_primary_final tkg2
	ON tkg.price_year = tkg2.price_year + 1
...
table_comparison_2 AS (SELECT price_year, ROUND(avg(price_growth_percent), 2) AS average_growth_price, ROUND(avg(salary_growth_percent), 2) AS average_growth_salary
FROM table_comparison
GROUP BY price_year)
SELECT price_year AS 'year', average_growth_price, average_growth_salary,
	CASE 
